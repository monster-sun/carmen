...
void setposition(const geometry_msgs::Point& points)
{	
...
moveit::planning_interface::MoveGroup group("arm");
	...
	ROS_INFO("Reference Iframe: %s", group.getEndEffectorLink().c_str());
	...
	target_pose1.position.x = points.x;
	target_pose1.position.y = points.y;
	target_pose1.position.z = points.z;
...
{	
	...
	//Receive coordinate via topic 'point'
	ros::Subscriber coord = node_handle.subscribe("point", 1,setposition);
